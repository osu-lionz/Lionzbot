    ...
    run: async(client, message, args, container) => {
        ...
        message.channel.send("Searching top plays...").then(async msg => {
            ...
            const embed = new MessageEmbed()
                .setColor('#B65FCF')
                .setAuthor({ name: `Top osu!AutoPilot plays for ${playerinfo.data.player.info.name}`, iconURL: `https://a.leeuwz.nl/${playerinfo.data.player.info.id}`})
                .setDescription(description)
                .setThumbnail(`https://a.leeuwz.nl/${playerinfo.data.player.info.id}?${Math.floor(Math.random()*9999)}`);
